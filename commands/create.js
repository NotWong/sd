...
module.exports = class extends Command {
    ...
    async execute(message) {
        ...
        if (!match) return message.channel.send(`Uso: \`${this.client.config.prefix}creargiveaway [channel-mention|channel-id][\'-w <winner-count>\'] <#d#h#m#s|time-in-seconds> <giveaway-title>\``);
        ...
        if (filtered >= 5) return message.channel.send(`\`ERR:\` Solo se pueden activar 5 GIVEAWAYS.`);
